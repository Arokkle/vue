...
const activeIndex = ref('recommended')
...
const getRecommendedArticles = async () => {
  try {
    const res = await articleStore.getPersonalizedRecommendations(20)
    articles.value = res.articles
  } catch (error) {
    console.error('获取推荐文章失败:', error)
    // 失败时回退到旧的推荐API
    try {
      await articleStore.getRecommendedArticles()
...
const handleSizeChange = async (size) => {
  try {
    const res = await articleStore.getPersonalizedRecommendations(size)
    articles.value = res.articles
  } catch (error) {
    console.error('获取推荐文章失败:', error)
    // 失败时回退到旧的推荐API
...
const handleCurrentChange = async (page) => {
  try {
    // 新的推荐API暂时不支持分页，使用固定大小
    const res = await articleStore.getPersonalizedRecommendations(articleStore.size)
    articles.value = res.articles
  } catch (error) {
    console.error('获取推荐文章失败:', error)
    // 失败时回退到旧的推荐API
